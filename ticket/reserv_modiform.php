	...
	<form name='reserv_form' method='post' action='reserv_mod.php'>
		<div class="ticket_form">
				...
				<option value=''>회차선택</option>		  
						...
						$stdate = mysql_result($modresult, $i, 2);
							echo"<option value='$res'";?>
							<? if($restic == $res)
								{ echo " selected='selected'" ;
							echo">$res 회 [ "?><?=$stdate." ]</option>";
						}
						?><? if($restic == $res)
							{ echo "selected='selected'" ;}
						echo">$res 회 ["?><?=$stdate."]</option>";
			...
			<div class="reserve_modify"><p class="line_title">연락처</p><input type='text' name='re_phone' value='<?echo $re_phone?>'></div>
			<div class="subbtn">
